...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="Emisor">
		...
		<xsl:if test="./ExpedidoEn">
			<xsl:call-template name="Domicilio">
				<xsl:with-param name="Nodo" select="./ExpedidoEn" />
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="Receptor">
		...
		<xsl:call-template name="Domicilio">
			<xsl:with-param name="Nodo" select="./Domicilio" />
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template match="Concepto">
		...
		<xsl:if test="./CuentaPredial">
			<xsl:apply-templates select="./CuentaPredial" />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="Formato"><xsl:param name="valor" />|<xsl:call-template name="Trim">
			<xsl:with-param name="s" select="$valor" />
		</xsl:call-template></xsl:template>

	<xsl:template name="Opcional">
		<xsl:param name="valor" />
		<xsl:if test="$valor">|<xsl:call-template name="Trim">
				<xsl:with-param name="s" select="$valor" />
